...
## 1. 数学：禁止题面剧透答案 / 关键中间步骤（V3.12.4 扩展）

**4 类剧透模式（V3.10 入库实测发现）：**

### 模式 1：题面格式提示 = 答案
- 例（违规）：`化简比并求比值：4.5∶(4/9)。请填(写法如「81:8,81/8」)`
- 答案 = `81:8,81/8` 完全是格式提示

### 模式 2：题面写解题公式
- 例（违规）：`求圆锥体积公式 V = πr²h/3，已知 r=2, h=3，V=?`
- 公式即解题路径，纯送分

### 模式 3：题面写答案数值
- 例（违规）：`把 3/20 化成小数，结果是 ___。已知 3/20 = 0.15。`

### 模式 4（V3.12 第二轮发现，最隐蔽）：剧透关键中间步骤
- 例（违规）：`20 以内既是奇数又是合数的数（9 和 15），它们的最大公因数是( )，最小公倍数是( )。`
  - 原题应该是 3 个空：第一空让用户**先想到** "9 和 15"（这本身是 R2-R3 思维），再求公因数公倍数
  - 入库 agent 删了第一空 + 把答案"9 和 15"塞进括号 → R3 难度降到 R2
- 模式 4 触发：题面括号 `( )` 或 `（ ）` 内含答案 / 提示词
- **修法**：补回原题被吞掉的空 + answer 加上对应答案

**规则：**
- `content` 字段不能含答案文本（`answer` 中的任何 alt_answer 都不行）
- `content` 也不能含 **关键中间推理步骤**（即被入库 agent 用括号括注 / 提示性描述吞掉的原题空）
- `explanation` 中可以、且应当含完整推导
- 真题原卷如此剧透 → 改写题面隐藏答案部分，或改为 choice（4 选项打散）
- 公式题：题面只给条件 + 提问，不给公式；公式归 explanation
- **审核时自查（V3.12.4 加强）**：
  - 把 `answer` 的每段 alt_answer 拿去 grep 整个 `content`，命中即违规
  - **入库前对照原 raw.txt** 字符级 diff，找"原题空数 != 入库空数"或"括号内多了原题没有的内容"

**例外：**
- 题面定义"设 V 为体积"这类**变量声明**不算剧透
- 给一个示例帮助理解（如"例：3/20 表示分子 3 除以分母 20"）→ 不算剧透，但要审视示例不是答案本身
- 候选集合必须列出（如"在「正比例 / 反比例 / 不成比例」中选"）→ 不算剧透
