  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: Icon(
          svc.selectedDateHasPlans ? Icons.tune : Icons.add,
          color: Colors.white,
        ),
      ),
    );
...
class _DayGroupCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final done = group.items
        .where((i) => i.status == PlanItemStatus.completed)
        .length;
    return Card(
      elevation: 2,
      child: Column(
...
class _ItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      dense: true,
      leading:
          Text(item.subjectEmoji, style: const TextStyle(fontSize: 22)),
      title: Text(
        item.displayTitle,
        style: TextStyle(
...
// ── Week plan card (expanded, day by day) ──────
class _WeekPlanCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Map day plans by date string for fast lookup
    final dayMap = {
      for (final c in group.children)
        c.startDate.toIso8601String().substring(0, 10): c
    };
    final days =
        PlanDateUtils.daysInRange(group.startDate, group.endDate);
            ...
            return Column(
              ...
              children: [
                ...
                if (dayPlan == null || dayPlan.items.isEmpty)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(24, 2, 12, 6),
                    child: Text('暂无内容',
                        style: TextStyle(
                            fontSize: 12,
...
// ── Month plan card (per-week summaries) ───────
    ...
    return Card(
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 1, indent: 12, endIndent: 12),
          // Per-week breakdown
          ...group.children.map((wg) {
            final wItems = wg.allItems;
            final wDone = wItems
                .where((i) => i.status == PlanItemStatus.completed)
                .length;
            return ListTile(
              ...
              leading: const Icon(Icons.calendar_view_week,
                  size: 16, color: Colors.deepPurple),
              title: Text(
                PlanDateUtils.weekLabel(wg.startDate, wg.endDate),
...
// ═══════════════════════════════════════════════
//  New plan creation flow (when no plans exist)
// ═══════════════════════════════════════════════
    ...
    switch (_type) {
      ...
      case PlanGroupType.week:
        if (await svc.checkWeekOverlap(_date)) {
          ...
          await svc.createWeekPlan(_date, drafts);
        }
      case PlanGroupType.month:
        if (await svc.checkMonthOverlap(_date)) {
          err = '月份重叠：该日期范围已有月计划';
        } else {
          await svc.createMonthPlan(_date, drafts);
        }
    }
    ...
    switch (_type) {
      case PlanGroupType.day:
        result[DateFormat('M月d日 (EEEE)', 'zh_CN').format(_date)] =
            drafts.toList();
      case PlanGroupType.week:
        final ws = PlanDateUtils.dateOnly(_date);
        final we = PlanDateUtils.weekEnd(_date);
        ...
        for (var i = 0; i < days.length; i++) {
          if (byDay[i].isNotEmpty) {
            result[DateFormat('M月d日 (EEEE)', 'zh_CN').format(days[i])] =
                byDay[i];
          }
        }
      case PlanGroupType.month:
        ...
        for (var wi = 0; wi < weeks.length; wi++) {
          ...
          for (var di = 0; di < days.length; di++) {
            if (byDay[di].isNotEmpty) {
              result[DateFormat('M月d日 (EEEE)', 'zh_CN').format(days[di])] =
                  byDay[di];
            }
          }
        }
