...
class VisionOcrService {
  ...
  Future<String?> recognize(Uint8List png, {String hint = ''}) async {
    ...
    const sys = '你是手写识别助手，把图片里学生的手写作答**忠实转写成文字**，供老师批改。规则：'
        '①手写文字/算式/数字 → 原样识别（分数写 a/b）；'
        '②证明/解答过程 → 逐行转写全部推理步骤，辅助线作法等用文字写出（如"连接AD""过C作CE⊥AB"）；'
        '③纯几何图形/作图 → 客观描述画了什么（图形、关键顶点/位置、比例、对称轴、辅助线位置）。'
        '只输出转写/描述的内容本身，不解释、不判分、不加"识别结果"等前缀。';
