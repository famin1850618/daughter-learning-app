...
OUT_PATH = os.path.join(ROOT, 'calibration_log/d2_math_review_v3.jsonl')
...
def combine_v2(round_per_dim: dict) -> dict:
    """V3 combine: mental_flex dampening + V3 改动 #1 distractor 单维保护.
    ...
    V3 改动 #1: distractor 单维拉高保护
      - 如果 max(step, mental, calc, kp) <= 2（其他维都 ≤ R2）
        且 distractor > 2 → distractor 上限 R2
      - Famin V2 verify B 组 4/5 道偏高 1-2 档都是 distractor=3 单维拉高
    """
    rpd = dict(round_per_dim)
    mental = rpd.get('mental')
    step = rpd.get('step')

    # dampening (V2 carryover)
    ...
    if distractor is not None:
        ...
        if non_distractor_max <= 2 and distractor > 2:
            # 其他维都 R1-R2 → distractor 上限 R2（即使陷阱密度 0.75+）
            rpd['distractor'] = 2
            rpd['_distractor_capped'] = True  # 标记便于 reasoning

    rounds = [r for r in rpd.values() if r is not None and not isinstance(r, bool)]
    if not rounds:
